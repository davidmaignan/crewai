...
from meeting_minutes.crews.gmailcrew.tools.gmail_tool import GmailTool
...
class GmailCrew():
	...
	def gmail_draft_agent(self) -> Agent:
		return Agent(
			config=self.agents_config['gmail_draft_agent'], # type: ignore
			tools=[GmailTool()],
			verbose=True
		)
	...
	def gmail_draft_task(self) -> Task:
		return Task(
			config=self.tasks_config['gmail_draft_task'], # type: ignore
		)
